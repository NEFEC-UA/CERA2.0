##CERA2.0=group
##[CAll] Float to Int (1 to 5)=name
##Raster_in_float=raster
##Raster_to_int=output raster
...
def designformula(entries):
    formula = '(' + str(entries[0].ref) + ' > 0 AND ' + str(entries[0].ref) + ' < 1.5)*1 + (' + str(entries[0].ref) + ' >= 1.5 AND ' + str(entries[0].ref) + ' < 2.5)*2 + (' + str(entries[0].ref) + ' >= 2.5 AND ' + str(entries[0].ref) + ' < 3.5)*3 + (' + str(entries[0].ref) + ' >= 3.5 AND ' + str(entries[0].ref) + ' < 4.5)*4 + (' + str(entries[0].ref) + ' >= 4.5)*5'
    print formula
    return formula


# Get layer object
rasterfloat = processing.getObject(Raster_in_float)

#Layer string and entries
layers = [rasterfloat] #make a string of layers
