##CERA2.0=group
##[CVal] PopdenInf + Eco=name
##PopdenInf=raster
##Ecology=raster
##Coastal_Value=output raster
...
def calculator(layers, entries):
    ...
    path = str(Coastal_Value) # name of the output
...
def designformula(entries):
    formula = str(entries[0].ref) + ' + ' + str(entries[1].ref) + ' - 1'
    print formula
    return formula


# Get layer object
popden_inf = processing.getObject(PopdenInf)
ecology = processing.getObject(Ecology)

#Layer string and entries
layers = [popden_inf, ecology] #make a string of layers
